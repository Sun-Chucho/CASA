...
export default function HotelTabs() {
  ...
  const handleInstallApp = async () => {
    if (installPrompt) {
      ...
      setInstallFeedback("Please use your browser's menu to install CASSA Hotel as a PWA app.");
    }
  };
  ...
  return (
    <main
      ...
      style={{
        backgroundImage: "linear-gradient(rgba(10, 10, 10, 0.96), rgba(10, 10, 10, 0.96)), url('/logo.png')",
      ...
      <div className="w-full max-w-lg rounded-3xl bg-slate-950/95 backdrop-blur shadow-2xl overflow-hidden border border-slate-700 text-white">
        <div className="p-6 text-center border-b border-slate-800 bg-slate-900/90">
          <div className="mx-auto mb-4 flex h-20 w-20 items-center justify-center rounded-2xl bg-white shadow-2xl overflow-hidden">
            ...
            <img src="/logo.png" alt="CASSA Hotel logo" className="h-full w-full object-contain" />
          </div>
          <h1 className="text-2xl font-black tracking-tight text-white">CASSA HOTEL</h1>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-h-[22rem] overflow-y-auto pr-1">
            {links.map((link) => (
              <Link
                key={link.name}
                href={link.url}
                className="flex items-center justify-center p-4 rounded-xl font-bold text-center border border-slate-700 bg-slate-900/70 text-slate-100 hover:bg-white hover:text-slate-950 hover:border-white/80 transition-all duration-200"
              >
                {link.name}
              </Link>
            ))}
            ...
            <div className="mt-6 pt-5 border-t border-slate-800">
              <div className="bg-slate-900/70 rounded-xl p-4 border border-slate-700 text-left">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <h3 className="text-xs font-bold uppercase tracking-[0.3em] text-white">Install CASSA Hotel App</h3>
